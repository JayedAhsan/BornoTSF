...
LRESULT CALLBACK BornoTSF_WindowProc(HWND wndHandle, UINT uMsg, WPARAM wParam, LPARAM lParam);

class BornoTSF : public ITfTextInputProcessorEx,
...
public:
    ...
    HRESULT _HandleCandidateSelect(TfEditCookie ec, _In_ ITfContext *pContext, UINT code);
...
private:
    ...
    BOOL _AddTextProcessorEngine();                          // startup path: queries active profile
    BOOL _AddTextProcessorEngine(LANGID langid, REFGUID guidProfile); // OnActivated path: GUID known
    BOOL VerifyBornoTSFCLSID(_In_ REFCLSID clsid);

    friend LRESULT CALLBACK BornoTSF_WindowProc(HWND wndHandle, UINT uMsg, WPARAM wParam, LPARAM lParam);
