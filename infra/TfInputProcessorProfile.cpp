...
HRESULT CTfInputProcessorProfile::GetActiveLanguageProfile(REFCLSID rclsid, _Out_ LANGID *plangid, _Out_ GUID *pguidProfile)
{
    if (_pInputProcessorProfile)
    {
        return _pInputProcessorProfile->GetActiveLanguageProfile(rclsid, plangid, pguidProfile);
    }
    else
    {
        return E_FAIL;
